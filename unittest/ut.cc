...
TEST(nvlogtest, log_ops)
{
	...
	ASSERT_NE((nvm_root_obj = nvm_init_heap(
			   NVHEAP_POOL_PATH, NVHEAP_POOL_SIZE, &need_recovery)),
		  nullptr)
		<< "nvm_init_heap failed\n";
	ASSERT_NE(nvlog_init(nvm_root_obj), 0) << "init_nvlog_init failed\n";
	ASSERT_EQ(nvlog_create(NULL, &nvlog, TS_OPLOG_SIZE, type,
			       STATUS_NVLOG_NORMAL),
		  0)
		<< "nvlog_create failed\n";
	operation.a = 0;
...
TEST(nvlogtest, variable_log_size)
{
	...
	for (i = 3; i < 19; i++) {
		nvlog_size = 1 << i;
		ASSERT_NE((nvm_root_obj = nvm_init_heap(NVHEAP_POOL_PATH,
							NVHEAP_POOL_SIZE,
							&need_recovery)),
			  nullptr)
			<< "nvm_init_heap failed\n";
		ASSERT_NE(nvlog_init(nvm_root_obj), 0)
			<< "init_nvlog_init failed\n";
		ASSERT_EQ(nvlog_create(NULL, &nvlog, nvlog_size, type,
				       STATUS_NVLOG_NORMAL),
			  0)
			<< "nvlog_create failed\n";
		operation.a = 0;
		...
		for (i = 0; i < MAX_LOOP; i++) {
			enq_num = deq_num = 0;
			while ((nvl_entry_hdr =
					nvlog_enq(&nvlog, sizeof(operation)))) {
				nvl_entry_hdr->wrt_clk = enq_num;
				memcpy((void *)&nvl_entry_hdr->obj,
				       (void *)&operation, sizeof(operation));
				enq_num++;
			}
			nvlog_enq_persist(&nvlog);
			while ((nvl_entry_hdr = nvlog_deq(&nvlog))) {
...
TEST(nvlogtest, variable_obj_size)
{
	...
	for (i = 0; i < MAX_LOOP; i++) {
		...
		while ((nvl_entry_hdr = nvlog_enq(&nvlog, obj_size))) {
			...
			memcpy((void *)&nvl_entry_hdr->obj, (void *)obj,
			       obj_size);
			enq_num++;
		}
		nvlog_enq_persist(&nvlog);
...
TEST(oplog_test, test_enq_deq)
{
	...
	ASSERT_NE((nvm_root_obj = nvm_init_heap(
		...
		<< "nvm_init_heap failed\n";
	ASSERT_NE(nvlog_init(nvm_root_obj), 0) << "init_nvlog_init failed\n";
	ASSERT_EQ(oplog_create(NULL, &oplog, TS_OPLOG_SIZE,
			       STATUS_NVLOG_NORMAL),
		  0)
		<< "oplog_create failed\n";
	for (i = 0; i < MAX_LOOP; i++) {
		...
		enq_num = deq_num = 0;
		while ((op_entry = oplog_enq(&oplog, enq_num, deq_num,
					     &op_info))) {
			memcpy((void *)&op_entry->oplog_hdr.opd, (void *)op,
			       op_size);
			enq_num++;
		}
...
TEST(ckptlog_test, test_enq_deq)
{
	...
	chs->cpy_hdr.p_act_vhdr =
		(ts_act_vhdr_t *)malloc(sizeof(ts_act_vhdr_t));
	chs->cpy_hdr.p_act_vhdr->np_org_act = ahs->obj_hdr.obj;

	ASSERT_EQ(ckptlog_create(NULL, &ckptlog, TS_CKPTLOG_SIZE,
				 STATUS_NVLOG_NORMAL),
		  0)
	...
	for (i = 0; i < MAX_LOOP; i++) {
		seq_num = 0;
		while ((ckpt_entry = ckptlog_enq(&ckptlog, seq_num, seq_num + 1,
						 chs))) {
			seq_num++;
		}
...
TEST(oplog_test, recovery)
{
	...
	ASSERT_NE((nvm_root_obj = nvm_init_heap(
		...
		<< "nvm_init_heap failed\n";
	ASSERT_NE(nvlog_init(nvm_root_obj), 0) << "init_nvlog_init failed\n";
	ASSERT_EQ(oplog_create(NULL, &oplog1, TS_OPLOG_SIZE,
			       STATUS_NVLOG_NORMAL),
		  0)
		<< "oplog1_create failed\n";
	ASSERT_EQ(oplog_create(NULL, &oplog2, TS_OPLOG_SIZE,
			       STATUS_NVLOG_NORMAL),
		  0)
		<< "oplog2_create failed\n";
	ASSERT_EQ(oplog_create(NULL, &oplog3, TS_OPLOG_SIZE,
			       STATUS_NVLOG_NORMAL),
		  0)
		<< "oplog3_create failed\n";
	ASSERT_EQ(oplog_create(NULL, &oplog4, TS_OPLOG_SIZE,
			       STATUS_NVLOG_NORMAL),
		  0)
...
TEST(ckptlog_test, recovery)
{
	...
	chs6->cpy_hdr.p_act_vhdr =
		(ts_act_vhdr_t *)malloc(sizeof(ts_act_vhdr_t));
	chs6->cpy_hdr.p_act_vhdr->np_org_act = ahs3->obj_hdr.obj;

	/* create ckptlog*/
	ASSERT_EQ(ckptlog_create(NULL, &ckptlog2, TS_CKPTLOG_SIZE,
				 STATUS_NVLOG_NORMAL),
		  0)
		<< "ckptlog_create failed\n";
	ASSERT_EQ(ckptlog_create(NULL, &ckptlog, TS_CKPTLOG_SIZE,
...
TEST(cpp, btree_ins_read_retry_read)
{
	ts::ts_persistent_ptr<btree> p_btree;
	char input_val[5] = "ABCD";
	char *val, *ret_val;
	...
	for (int i = 0; i < read_miss_imm; ++i) {
		std::cout << "*** reading key " << miss_imm[i] << " ***"
			  << std::endl;
		ts::ts_txn::run_rw(ts::snapshot, [&] {
			ret_val = p_btree->lookup(miss_imm[i]);
			if (ret_val != val) {
				++read_miss_retry;
			}
			ASSERT_EQ(ret_val, val);
	...
	std::cout << "list of missed keys" << std::endl;
	for (int i = 0; i < read_miss_imm; ++i) {
		std::cout << miss_imm[i] << std::endl;
	}
	std::cout << "*** read misses_imm= " << read_miss_imm << " ***"
		  << std::endl;
	std::cout << "*** read misses_imm_retry= " << read_miss_imm_retry
		  << " ***" << std::endl;
	std::cout << "*** read misses_retry= " << read_miss_retry << " ***"
		  << std::endl;
...
TEST(cpp, btree_concurrent_insert)
{
	ts::ts_persistent_ptr<btree> p_btree;
	...
	for (int i = 0; i < N_THREADS; i++) {
		...
		threads.push_back(ts::ts_thread(ut_worker_insert, p_btree,
						start_key[i], end_key[i], i,
						val));
	}
	go();
	for (auto &th : threads) {
...
TEST(cpp, btree_concurrent_lookup)
{
	ts::ts_persistent_ptr<btree> p_btree;
	char input_val[5] = "ABCD";
	char *val, *ret_val;
	...
	for (int i = 0; i < n_threads_insert; ++i) {
		std::cout << "creating thread " << i << std::endl;
		i_threads.push_back(ts::ts_thread(ut_worker_insert, p_btree, 0,
						  KEYS, i, val));
	}
	go();
	...
	for (int i = 0; i < N_THREADS; ++i) {
		std::cout << "creating thread " << i << std::endl;
		l_threads.push_back(ts::ts_thread(ut_worker_lookup, p_btree,
						  start_key[i], end_key[i], i,
						  val, read_miss));
	}
	go();
...
/*TEST(cpp, btree_concurrent)
{
	...
	for (int i = 0; i < N_THREADS; ++i) {
		std::cout << "creating thread " << i << std::endl;
		i_threads.push_back(ts::ts_thread(ut_worker_insert, p_btree,
						  start_key[i], end_key[i], i,
						  val));
	}
	go();
