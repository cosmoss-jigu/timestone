...
#ifdef USE_NVM_DEVICE
	{
		/* check if the limitation of mmaped files
		 * are sufficiently large or not */
		FILE *fp;
		int num;
		...
		fclose(fp);
		if (num < 655300) {
			printf("NV-JEMALLOC: max_map_count is too small\n");
			printf("NV-JEMALLOC: increase it with the following command\n");
			printf("NV-JEMALLOC: $ sudo sysctl -w vm.max_map_count=655300\n");
			exit(EXIT_FAILURE);
		}
	}
	{
		/* creating directories to spread out files */
